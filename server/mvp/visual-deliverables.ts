...
export async function buildVisualDeliverables(input: {
  ...
  try {
    ...
    matrixGenerated = matrixQuality.pass && matrixQuality.relevant && matrixQuality.readable && matrixQuality.no_clipping && matrixQuality.matrix_representation && matrixQuality.no_internal_identifiers && matrixQuality.no_invented_results;
  ...
  const matrixDraft = byKey("consistency_matrix"); if (matrixDraft) { const table = matrixDraft.blocks.find((block) => block.kind === "table"); if (table?.kind === "table") { table.rows = consistencyTableRows(input.matrix, input.definition, input.design); table.render_hint = "compact_landscape"; table.title = "Matriz de consistencia editable"; } if (matrixGenerated) insertBeforeFirstTable(matrixDraft, [matrixBlock]); }
