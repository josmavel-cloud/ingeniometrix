...
export function responseCostBound(params: { model?: string | null; max_output_tokens?: number | null; [key: string]: unknown }) {
  ...
  if (unsupportedImage) return null;
  const inputTokens = Buffer.byteLength(serialized) + 2048 + imageTokens;
  const longContext = params.model === "gpt-5.4" && inputTokens > 272000;
  const maximumUsd = (inputTokens * rates[0] * (longContext ? 2 : 1) + params.max_output_tokens * rates[1] * (longContext ? 1.5 : 1)) / 1e6;
  return { maximumUsd, rates, inputTokens, imageTokens };
}
