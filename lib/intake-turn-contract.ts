...
export const intakeTurnInputSchema = z.object({ requestId: z.string().uuid(), baseRevision: z.number().int().positive(), etag: z.string().max(128), message: z.string().trim().min(1).max(4000) }).strict();
